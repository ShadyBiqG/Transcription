...
### Ошибка `charmap codec can't encode characters`

Это означает, что noScribe запущена службой с устаревшей системной кодировкой и не
может вывести распознанную кириллицу. В комплектном
`deploy\windows-service\TranscriptionService.xml` уже заданы `PYTHONUTF8=1` и
`PYTHONIOENCODING=utf-8`. Скопируйте актуальный XML на сервер и перезапустите службу.
