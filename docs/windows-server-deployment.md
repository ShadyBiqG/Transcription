...
Текущая версия `.gitignore` уже исключает `.env`, `TranscriptionService.exe`,
`data`, журналы и локальные материалы планирования в `specs`. JSON-схемы,
необходимые для запуска тестов на сервере, хранятся отдельно в
`tests\contract\schemas` и входят в Git-репозиторий.

Штатное безопасное обновление запускается из обычного PowerShell или `cmd`:
...
Если текущий процесс запущен без административных прав, сценарий сам покажет
запрос UAC. Подтвердите его и при необходимости введите данные локального
администратора. Если запрос отменить, обновление безопасно завершится до остановки
службы и изменения файлов.

Сценарий автоматически:

- запрещает обновление при локальных изменениях, detached HEAD или расхождении веток;
- не допускает два одновременных обновления;
- создаёт Git bundle, копию `.env` и согласованную копию файлов SQLite после
  остановки службы;
- выполняет только `git pull --ff-only`;
- синхронизирует зависимости строго по `uv.lock`;
- запускает Ruff и pytest до включения новой версии;
- возвращает production-окружение в состояние `--no-dev`;
- запускает службу только после успешных проверок и проверяет health API;
- сохраняет полный журнал вне Git-репозитория в
  `C:\ProgramData\LocalTranscriptionService\update-logs`.

После успешного запуска сценарий сохраняет фактически развёрнутый commit в
`C:\ProgramData\LocalTranscriptionService\deployed-commit.txt`. Поэтому обычный
запуск обнаружит ситуацию, когда `git pull` уже был выполнен вручную, но служба ещё
не перезапускалась, и самостоятельно завершит синхронизацию. Если удалённая ветка,
локальный код и эта отметка совпадают, сценарий ничего не останавливает.

Для осознанного повторного прогона зависимостей, тестов и перезапуска той же версии
используйте:

```powershell
.\deploy\windows-service\update-project.cmd -ForceSync
```

Тесты можно пропустить только при осознанном аварийном обновлении:

```powershell
.\deploy\windows-service\update-project.cmd -SkipTests
```

При ошибке до изменения кода сценарий пытается вернуть ранее работавшую службу.
После уже выполненного `git pull` разрушительный автоматический rollback не
производится: служба остаётся остановленной, а пути к backup и журналу выводятся
в консоль.

Ручная последовательность для диагностики:
...
### Атрибуция не запускает внешний провайдер

- Создайте администратора командой `uv run transcription-service --create-admin EMAIL`
  от учётной записи, под которой работает служба.
- В разделе «Администрирование» укажите название, OpenAI-compatible API URL и
  API-ключ. Получите каталог либо введите основную, резервную и разрешённые модели
  вручную, затем включите провайдера.
- Проверьте `TRANSCRIPTION_FFMPEG_PATH`; служба должна иметь доступ к `ffmpeg.exe`.
- Ключ DPAPI нельзя перенести под другую Windows-учётную запись: сохраните его заново.
