...
Первая часть помощника для записей телемостов: веб-сервис принимает `.webm` в локальной сети, последовательно запускает установленный noScribe и отдает VTT с таймкодами.
...
Архитектура всех четырех частей описана в [docs/architecture.md](docs/architecture.md), а спецификации находятся в `specs/002-*` — `specs/005-*`.
